...
The customer completes payment and sees confirmation in Shopify checkout. Local cart contents are retained when opening checkout so an interrupted purchase does not lose the selection. Each website checkout records its cart and item revisions in `checkoutPayments` and passes a server-generated `_pbs_checkout_id` Shopify attribute. The `orders/paid` webhook at `/api/shopify/webhooks/orders-paid` verifies Shopify's raw-body HMAC with the existing app client secret and removes the purchased revisions from the MongoDB cart. The browser reconciles that payment when returning, focusing the website, or while awaiting confirmation. Re-added or newly added items survive; repeated webhooks and delayed browser saves cannot restore purchased revisions. Cart IDs have a unique database index to serialize concurrent upserts. Existing purchases made before checkout tracking was added cannot be correlated retroactively.

The account page reads Shopify orders using a separate server-only Admin API connection, matching the current website user's verified email exactly. It does not cache customer order data in the shared catalog cache. Reorder reads and authorizes the original order server-side, keeps its variant batch counts, artwork and notes, then validates fresh availability/quantity rules and creates a new checkout using current Shopify prices. Customers review the total before paying. Direct account reorders do not consume the current shopping cart. No live order or payment is created by the automated unit tests.

Register the app's paid-order subscription after deploying the endpoint: `node --experimental-strip-types --conditions=react-server --env-file=.env.local scripts/setup-shopify-paid-webhook.mjs --apply`. Without `--apply`, the script only checks registration. The callback uses `https://packbrand.vercel.app/api/shopify/webhooks/orders-paid`, the existing Vercel production alias, because Shopify rejects callback hosts registered as store domains. Customer-facing URLs remain on packbrandsolutions.com. The receiver uses the app's existing `SHOPIFY_ADMIN_CLIENT_SECRET`, matching [Shopify's webhook verification](https://shopify.dev/docs/apps/build/webhooks/verify-deliveries). Run `npm run test:shopify` for unit tests. With a local server on port 3108, `node --env-file=.env.local scripts/verify-cart-payment.mjs` exercises signed HTTP deliveries and cart reconciliation using isolated temporary database records, then removes those records.
